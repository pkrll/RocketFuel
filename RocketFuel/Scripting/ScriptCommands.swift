...
final class RocketFuelScript: NSScriptCommand {
    ...
    @objc @MainActor var active: Bool {
    ...
    override func performDefaultImplementation() -> Any? {
        ...
        case "Toggle":
            Task {
                await RocketFuelScriptBridge.shared.toggle()
            }
        case "Duration":
            let minutes = (directParameter as? NSNumber)?.doubleValue ?? 1
            Task {
                await RocketFuelScriptBridge.shared.activate(forMinutes: minutes)
            }
        case "BatteryLevel":
            let level = (directParameter as? NSNumber)?.intValue ?? 0
            Task {
                await RocketFuelScriptBridge.shared.setBatteryLevel(level)
            }
        case "BatteryMode":
            let enabled = (directParameter as? NSNumber)?.boolValue ?? false
            Task {
                await RocketFuelScriptBridge.shared.setBatteryMode(enabled)
            }
        default:
            return false
        }
